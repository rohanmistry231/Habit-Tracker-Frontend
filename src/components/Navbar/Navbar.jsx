...
const Navbar = () => {
  ...
  const getLinkClass = (path) => {
    ...
    return isActive
      ? 'text-orange-500'  // Active link in orange
      : `${isDarkMode ? 'text-gray-300' : 'text-gray-800'} hover:text-orange-500`;  // Default link style
  };
  ...
  return (
    <nav className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-md fixed w-full top-0 z-50`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-14">
          <div className="flex-shrink-0">
            <Link to="/" className={`${isDarkMode ? 'text-orange-400' : 'text-orange-600'} text-3xl font-bold`}>
              Habit Tracker
            </Link>
          </div>
      ...
      <div
        className={`md:hidden ${isDarkMode ? 'bg-gray-800' : 'bg-white'} shadow-lg space-y-4 px-6 pt-4 pb-6 transition-transform duration-300 ease-in-out transform ${
      ...
      >
        {['/', '/habits', '/profile'].map((path, index) => (
          <Link
            key={path}
            to={path}
            className={`block ${location.pathname === path ? 'text-orange-500' : getLinkClass(path)} text-lg py-2`}
            onClick={toggleMenu}
            style={{
              animation: isOpen
